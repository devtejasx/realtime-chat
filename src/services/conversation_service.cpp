...
models::Conversation ConversationService::create(std::int64_t actor_id,
    ...
    const events::ConversationCreated conversation_created_event{
        .conversation_id = conversation.id,
        .actor_id = actor_id,
        .is_group = conversation.is_group(),
        .participant_ids = ids,
    };
    publisher().publish(conversation_created_event.to_event());
    return conversation;
}
...
void ConversationService::remove(std::int64_t actor_id, std::int64_t conversation_id) {
    ...
    const events::ConversationDeleted conversation_deleted_event{
        .conversation_id = conversation_id,
        .actor_id = actor_id,
    };
    publisher().publish(conversation_deleted_event.to_event());
}
...
models::ConversationParticipant ConversationService::add_member(std::int64_t actor_id,
    ...
    const events::MemberAdded member_added_event{
        .conversation_id = conversation_id,
        .member_id = user_id,
        .actor_id = actor_id,
    };
    publisher().publish(member_added_event.to_event());
    return *participant;
}
...
void ConversationService::remove_member(std::int64_t actor_id,
    ...
    const events::MemberRemoved member_removed_event{
        .conversation_id = conversation_id,
        .member_id = target_user_id,
        .actor_id = actor_id,
    };
    publisher().publish(member_removed_event.to_event());
}
