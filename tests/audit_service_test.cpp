...
TEST(AuditPolicy, RecordsSecurityRelevantEvents) {
    for (const EventType type : {EventType::kUserRegistered,
                                 EventType::kUserLoggedIn,
                                 EventType::kUserLoggedOut,
                                 EventType::kPasswordChanged,
                                 EventType::kProfileUpdated,
                                 EventType::kUserRoleChanged,
                                 EventType::kConversationCreated,
                                 EventType::kConversationDeleted,
                                 EventType::kMemberAdded,
                                 EventType::kMemberRemoved,
                                 EventType::kMessageDeleted,
                                 EventType::kAdminAction}) {
...
TEST(AuditPolicy, SkipsHighVolumeTraffic) {
    ...
    for (const EventType type : {EventType::kMessageSent,
                                 EventType::kMessageEdited,
                                 EventType::kUserOnline,
                                 EventType::kUserOffline,
...
TEST(AuditService, PersistsAnAuditableEvent) {
    ...
    const auto event = rtc::events::UserLoggedIn{.user_id = fixture.actor_id,
                                                 .username = "ada",
                                                 .ip = "203.0.113.7",
                                                 .user_agent = "curl/8"}
                           .to_event();
...
TEST(AuditService, IgnoresANonAuditableEvent) {
    AuditFixture fixture;
    const auto event =
        rtc::events::MessageSent{
            .message_id = 1, .conversation_id = 2, .sender_id = fixture.actor_id}
            .to_event();
...
TEST(AuditService, IsIdempotentOnEventId) {
    ...
    AuditFixture fixture;
    const auto event = rtc::events::PasswordChanged{.user_id = fixture.actor_id}.to_event();
...
TEST(AuditService, DenormalisesTheActorUsername) {
    ...
    AuditFixture fixture;
    fixture.service.record(rtc::events::PasswordChanged{.user_id = fixture.actor_id}.to_event());
    ASSERT_EQ(fixture.audit_repository.rows.size(), 1U);
...
TEST(AuditService, DerivesTheTargetPerEventType) {
    ...
    fixture.service.record(rtc::events::MemberAdded{
        .conversation_id = 77, .member_id = 5, .actor_id = fixture.actor_id}
                               .to_event());
    fixture.service.record(rtc::events::MessageDeleted{
        .message_id = 31, .conversation_id = 77, .actor_id = fixture.actor_id, .by_moderator = true}
                               .to_event());
    fixture.service.record(rtc::events::AdminAction{.actor_id = fixture.actor_id,
...
TEST(AuditService, PreservesTheModeratorFlagOnADeletion) {
    ...
    AuditFixture fixture;
    fixture.service.record(rtc::events::MessageDeleted{
        .message_id = 4, .conversation_id = 9, .actor_id = fixture.actor_id, .by_moderator = true}
                               .to_event());
    ASSERT_EQ(fixture.audit_repository.rows.size(), 1U);
...
TEST(AuditService, CarriesCorrelationIds) {
    ...
    EXPECT_EQ(fixture.audit_repository.rows.front().trace_id, "0af7651916cd43dd8448eb211c80319c");
}

TEST(AuditService, RecordsAnExplicitAdminAction) {
    AuditFixture fixture;
    EXPECT_TRUE(fixture.service.record_admin_action(
        fixture.actor_id, "user.ban", "user", "42", {{"reason", "spam"}}, "10.0.0.1", "curl"));
    ASSERT_EQ(fixture.audit_repository.rows.size(), 1U);
    const auto& row = fixture.audit_repository.rows.front();
...
TEST(AuditService, ProjectsARecordWithAStableKeySet) {
    AuditFixture fixture;
    fixture.service.record(rtc::events::UserLoggedIn{.user_id = fixture.actor_id}.to_event());
    const auto json = rtc::services::AuditService::to_json(fixture.audit_repository.rows.front());
