...
public:
    ...
    [[nodiscard]] virtual std::vector<std::int64_t> attachment_ids_for(
        std::int64_t message_id) = 0;
};
...
public:
    std::size_t link_to_message(std::int64_t, const std::vector<std::int64_t>&,
                                std::int64_t) override {
        return 0;
    }
    [[nodiscard]] std::vector<std::int64_t> attachment_ids_for(std::int64_t) override {
        return {};
    }
};
