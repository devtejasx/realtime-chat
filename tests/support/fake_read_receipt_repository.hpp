...
public:
    models::ReadReceipt upsert_state(std::int64_t message_id, std::int64_t user_id,
    ...
    void mark_conversation_read(std::int64_t, std::int64_t user_id,
    ...
    std::vector<models::ReadReceipt> list_for_message(std::int64_t message_id) override {
        ...
        for (const auto& r : receipts_)
            if (r.message_id == message_id) out.push_back(r);
        return out;
    }
